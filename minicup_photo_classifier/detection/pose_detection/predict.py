...
from minicup_photo_classifier import PACKAGE_ROOT

dir_path = os.path.dirname(os.path.realpath(__file__))
sys.path.append(dir_path + '/../lib/multicut_cython/')
from multicut import solve_nl_lmp

from .pose_dataset import get_pairwise_index
    ...
    def load(self):
        for cidx1 in range(self.num_keypoints):
            for cidx2 in range(cidx1 + 1, self.num_keypoints):
                model_name = "{}/spatial_model_cidx_{}_{}.mat".format(
                    join(
                        PACKAGE_ROOT,
                        self.cfg.pairwise_model_dir
                    ),
                    cidx1 + 1,
                    cidx2 + 1
                )
        ...
        if not self.cfg.tensorflow_pairwise_order:
            loaded = sio.loadmat(join(
                PACKAGE_ROOT,
                self.cfg.pairwise_stats_fn
            ))
            self.graph = loaded['graph']
            self.pairwise_means = loaded['means']
            self.pairwise_std_devs = loaded['std_devs']
